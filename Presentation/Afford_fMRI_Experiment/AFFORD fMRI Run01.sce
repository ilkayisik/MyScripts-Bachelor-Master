...
int repetitionCount = 2; #all stimuli will be shown 2 times

loop until (pulse_manager.main_pulse_count() == 1)
begin
end;

array <int> orders[124] = {1,0,8,1,4,5,0,2,0,2,2,0,5,2,0,8,3,8,0,4,3,0,8,1,7,0,2,6,0,3,5,0,7,8,5,0,5,4,0,6,4,6,3,0,1,0,7,0,5,0,2,0,3,0,6,5,7,2,3,0,5,8,0,1,0,8,0,6,7,1,0,1,0,6,0,4,8,0,7,0,6,4,7,0,3,8,0,1,3,7,6,0,5,0,1,0,6,0,3,0,2,4,0,8,0,4,2,0,3,0,5,6,0,2,0,4,0,7,4,0,7,0,1,0};
#array for the first_response order of the trials from optseq2
array <int> durations[124] = {5000,2500,5000,5000,5000,5000,2500,5000,2500,5000,5000,5000,5000,5000,5000,5000,5000,5000,2500,5000,5000,7500,5000,5000,5000,2500,5000,5000,5000,5000,5000,5000,5000,5000,5000,2500,5000,5000,2500,5000,5000,5000,5000,2500,5000,2500,5000,2500,5000,7500,5000,5000,5000,7500,5000,5000,5000,5000,5000,5000,5000,5000,2500,5000,2500,5000,7500,5000,5000,5000,2500,5000,2500,5000,2500,5000,5000,17500,5000,5000,5000,5000,5000,5000,5000,5000,10000,5000,5000,5000,5000,2500,5000,7500,5000,5000,5000,2500,5000,5000,5000,5000,2500,5000,2500,5000,5000,2500,5000,2500,5000,5000,5000,5000,7500,5000,5000,5000,5000,5000,5000,2500,5000,7500};
#array for the durations of the events

array <int> ii[8][repetitionCount*5]; # creates a two dimentional array for 10 stimuli for 8 trial
# (8 10 item array to put the 10 stimuli inside)
array <int> pp[8]; # 8 item array to increment in order to chose different stimuli everytime for each trial from the array belongs to that trial
...
###########################                                
output_file orders_Run1 = new output_file;
orders_Run1.open( "orders_Run1.txt" ); 
...
###########################
output_file durations_Run1 = new output_file;
durations_Run1.open( "durations_Run1.txt" ); 
...
until
	i > 124
begin
	durations_Run1.print(durations[i]);
	durations_Run1.print( "\n" );  
	i = i + 1
end;
durations_Run1.close();
###########################

###########################
output_file ii_Run1 = new output_file;
ii_Run1.open( "ii_Run1.txt" ); 
...
until
	i > 10
begin
	ii_Run1.print(ii[1][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[2][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[3][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[4][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[5][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[6][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[7][i]);
	ii_Run1.print( "\t" );  
	ii_Run1.print(ii[8][i]);
	ii_Run1.print( "\n" );
	i = i + 1
end;
ii_Run1.close();
...
###########################
output_file pp_Run1 = new output_file;
pp_Run1.open( "pp_Run1.txt" ); 
...
###########################
output_file lll_Run1 = new output_file;
lll_Run1.open( "lll_Run1.txt" ); 
...
output_file exp_Run1 = new output_file;
exp_Run1.open( "exp_Run1.txt" ); 
